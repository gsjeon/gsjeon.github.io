...
**즉, kubelet이 nodeStatusUpdateFrequency(10초) 에 의해 4번(40초) 동안 노드
상태를 업데이트 하지 못하면, Controller-mnager의
--node-monitor-grace-period(40초) 에 의해 노드는 <mark>Unknown</mark> 상태가
된다.**
...
**즉, 노드가 죽으면 "pod-eviction-timeout" 에 설정된 시간(5분)이 지나야 파드가 다른 노드로
옮겨진다.**
