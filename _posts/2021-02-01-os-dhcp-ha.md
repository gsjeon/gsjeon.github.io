...
가 실행중이다.
```
[clex@taco2-master1 ~]$ openstack network agent list --sort-column 'Agent Type'
+--------------------------------------+--------------------+-----------------------------+-------------------+-------+-------+---------------------------+
| ID                                   | Agent Type         | Host                        | Availability Zone | Alive | State | Binary                    |
+--------------------------------------+--------------------+-----------------------------+-------------------+-------+-------+---------------------------+
| 15dbe355-43a1-44e3-bfe5-a9f21e373fcb | DHCP agent         | taco2-master3               | nova              | :-)   | UP    | neutron-dhcp-agent        |
| 272c58fa-e325-4c53-8c5c-7ce16a13416b | DHCP agent         | taco2-master2               | nova              | :-)   | UP    | neutron-dhcp-agent        |
| e866c41c-5871-4328-8d89-0ac4d35ffb3f | DHCP agent         | taco2-master1               | nova              | :-)   | UP    | neutron-dhcp-agent        |
...
```

위와 같은 상태에서 네트워크를 만들면 아래와 같이 <mark>1개</mark>의 dhcp agent 가 할당된다.
```
[clex@taco2-master1 ~]$ openstack network agent list --network public-net
+--------------------------------------+------------+---------------+-------------------+-------+-------+--------------------+
| ID                                   | Agent Type | Host          | Availability Zone | Alive | State | Binary             |
+--------------------------------------+------------+---------------+-------------------+-------+-------+--------------------+
| e866c41c-5871-4328-8d89-0ac4d35ffb3f | DHCP agent | taco2-master1 | nova              | :-)   | UP    | neutron-dhcp-agent |
...
* 이것은 <mark> 단일 DHCP </mark> 라고 하겠다.

위 public-net 에 dhcp agent를 추가 할당할 수 있다.
```
[clex@taco2-master1 ~]$ openstack network agent add network --dhcp e866c41c-5871-4328-8d89-0ac4d35ffb3f public-net

[clex@taco2-master1 ~]$ openstack network agent list --network public-net
+--------------------------------------+------------+---------------+-------------------+-------+-------+--------------------+
| ID                                   | Agent Type | Host          | Availability Zone | Alive | State | Binary             |
+--------------------------------------+------------+---------------+-------------------+-------+-------+--------------------+
| b30be7b1-da0f-461d-b2c5-39bf0384fddb | DHCP agent | taco2-master2 | nova              | :-)   | UP    | neutron-dhcp-agent |
| e866c41c-5871-4328-8d89-0ac4d35ffb3f | DHCP agent | taco2-master1 | nova              | :-)   | UP    | neutron-dhcp-agent |
+--------------------------------------+------------+---------------+-------------------+-------+-------+--------------------+
```
* public-net 에 dhcp agent 를 추가 할당한 모습이다.
* 이것은 <mark> 다중 DHCP /mark> 라고 하겠다.
...
+--------------------------------------+------------+---------------+-------------------+-------+-------+--------------------+
```
* <mark>public-net</mark> 이라는 네트워크의 DHCP Agent 는 <mark>taco2-master1</mark> 노드에서 실행 중.


아래와 같이 DHCP 에이전트를 죽였다.
```
[clex@taco2-master1 ~]$ openstack network agent set e866c41c-5871-4328-8d89-0ac4d35ffb3f --disable 
```

이후 상황을 살펴보니 taco2-master2 가 서비스를 이어간다.
```
[clex@taco2-master1 ~]$ openstack network agent list --network public-net
+--------------------------------------+------------+---------------+-------------------+-------+-------+--------------------+
| ID                                   | Agent Type | Host          | Availability Zone | Alive | State | Binary             |
...
* <mark>이전 taco2-master1</mark> 에서 작동하던 DHCP Agent는 사라졌고, <mark>신규 taco2-master2</mark> 가
  서비스를 이어간다.

그럼 이전 taco2-master1이 가지고 있던 IP 임대 정보는 어떻게 될까?
```

[clex@taco2-master1 ~]$ kubectl get po -n openstack -o wide |grep dhcp |grep master2
neutron-dhcp-agent-default-9h8qf                     1/1     Running     1          49m     10.3.1.152       taco2-master2   <none>           <none>

[clex@taco2-master1 ~]$ kubectl exec -ti -n openstack neutron-dhcp-agent-default-9h8qf -- cat /var/lib/neutron/dhcp/1492873a-519c-4e8e-bee9-744fe1bfea83/leases
...
* <mark>이전 taco2-master1</mark>가 IP를 임대했던 리스 정보도 모두 <mark>신규 taco2-master2</mark> 로 잘 이전된다.
...
```
()[neutron@taco2-master1 1492873a-519c-4e8e-bee9-744fe1bfea83]$ cat leases 
...
()[neutron@taco2-master2 1492873a-519c-4e8e-bee9-744fe1bfea83]$ cat leases 
1611375601 fa:16:3e:08:23:12 10.3.4.14 host-10-3-4-14 *
1611375601 fa:16:3e:25:cd:2b 10.3.4.12 host-10-3-4-12 *
1611375601 fa:16:3e:87:e3:24 10.3.4.10 host-10-3-4-10 *
1611375601 fa:16:3e:b3:16:3f 10.3.4.11 host-10-3-4-11 *
...
(master1)
```
[clex@taco2-master1 ~]$ sudo ip netns exec qdhcp-1492873a-519c-4e8e-bee9-744fe1bfea83 sudo tcpdump -i ns-40ab742e-a7 -n port 67 and port 68
...
(master2)
```
[clex@taco2-master2 ~]$ sudo ip netns exec qdhcp-1492873a-519c-4e8e-bee9-744fe1bfea83 sudo tcpdump -i ns-2ccfcf30-34 -n port 67 and port 68
...
```
[clex@taco2-master2 ~]$ openstack network agent set b30be7b1-da0f-461d-b2c5-39bf0384fddb --disable

[clex@taco2-master1 ~]$ openstack network agent list --network public-net
+--------------------------------------+------------+---------------+-------------------+-------+-------+--------------------+
| ID                                   | Agent Type | Host          | Availability Zone | Alive | State | Binary             |
+--------------------------------------+------------+---------------+-------------------+-------+-------+--------------------+
| b30be7b1-da0f-461d-b2c5-39bf0384fddb | DHCP agent | taco2-master2 | nova              | :-)   | DOWN  | neutron-dhcp-agent |
| e866c41c-5871-4328-8d89-0ac4d35ffb3f | DHCP agent | taco2-master1 | nova              | :-)   | UP    | neutron-dhcp-agent |
+--------------------------------------+------------+---------------+-------------------+-------+-------+--------------------+
```
